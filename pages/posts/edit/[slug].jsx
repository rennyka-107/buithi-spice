...
export default withAuth(function EditPost() {
  ...
  useEffect(async () => {
    if (slug) {
      try {
        ...
        const postRes = PostApi.getPostBySlug(slug);
        const result = await Promise.all([cateRes, postRes]);
        const [resultCate, resultPost] = result;
        if (resultPost.data && resultPost.data.status) {
          const { post } = resultPost.data;
